...
# Compute centroids
centroids <- st_centroid(tx_counties_map)

# Extract longitude/latitude
coords <- st_coordinates(centroids)
tx_counties_map$longitude <- coords[, 1]
tx_counties_map$latitude <- coords[, 2]

# Prepare coordinate input
locations <- tx_counties_map[, c("longitude", "latitude")] |> st_drop_geometry()

# Initialize distance matrix
n <- nrow(locations)
dist_matrix <- matrix(NA, n, n, dimnames = list(tx_counties_map$County, tx_counties_map$County))
